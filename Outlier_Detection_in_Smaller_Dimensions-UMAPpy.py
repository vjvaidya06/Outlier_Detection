...
def calcOutliers(data_input, maxoutliers, columns, method):
...
if data is not None and (data.name).split(".")[-1] == "mat":
    ...
    if ("data" not in st.session_state or data.name is not st.session_state["data"]):
        ...
        calcOutliers(data_input, maxoutliers[0], len(data_input.columns), method)
    ...
    combined = json.dumps([st.session_state["olDRN"], st.session_state["olSN"], st.session_state["TSN"], maxoutliers[0]])
...
if data is not None and (data.name).split(".")[-1] == "json":
    ...
    fig.supxlabel("Dimensions Reduced To")
    fig.supylabel("Percentage of Outliers Retained")
    numofoutliers = st.slider("Number of outliers", min_value=1, max_value=int(d[3]), value=None, step=None, format=None, key=None, help=None, on_change=None, args=None, kwargs=None, disabled=False)
